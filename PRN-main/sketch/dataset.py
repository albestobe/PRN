...
class Dataset(torch.utils.data.Dataset):
    ...
    def load_item(self, index):
        ...
        for i in range(num_bilateral):
            img_color = cv2.bilateralFilter(img_color, d=15, sigmaColor=60, sigmaSpace=15)
        ...
        return self.to_tensor(img), self.to_tensor(img_color),self.to_tensor(mask)
    ...
    def load_list(self, path):
        if isinstance(path, str):
            ...
            if os.path.isdir(path):
                path = list(glob.glob(path + '/*.jpg')) + list(glob.glob(path + '/*.png'))+list(glob.glob(path + '/*/*.jpg')) + list(glob.glob(path + '/*/*.png'))
                path.sort()
                return path
...
def generate_stroke_mask(im_size, max_parts=7, maxVertex=25, maxLength=100, maxBrushWidth=24, maxAngle=360):
