...
class RFRNetModel():
    ...
    #    writer = SummaryWriter(log_dir="log_info")
        ...
        while self.iter<iters:
            for items in train_loader:
                gt_images,gt_images_gray, masks = self.__cuda__(*items)
                #print(masks.shape)
                masked_images = gt_images * masks
                masked_images_gray =gt_images_gray * masks[:,0:1,:,:]
                #print(masked_images.shape, masked_images_gray.shape)
                input_images=torch.cat((masked_images,masked_images_gray),1)
                #print(input_images.shape)
                masks=torch.cat((masks,masks[:,0:1,:,:]),1)
                self.forward(input_images, masks, masked_images_gray, gt_images_gray)
    ...
    def test(self, test_loader, result_save_path):
        ...
        for items in test_loader:
            ...
            masks1=masks[:,0:1,:,:]
            comp_B = fake_B * (1 - masks1) + gt_images_gray * masks1
            # print(comp_B.shape)
            # print(fake_B.shape)
            # print(masks1.shape)
            # print(gt_images_gray.shape)
            #viz.images(gt_images, win='a', env='main', opts=dict(title='Original Image'))
            #viz.images(masked_images, win='b', env='main', opts=dict(title='Damaged Areas'))
            #viz.images(torch.clamp(comp_B*255, 0, 255), win='c',env='main',opts=dict(title='Brightness Map Restoration'))
            ...
            for k in range(comp_B.size(0)):
                ...
                grid = make_grid(masked_images_gray[k:k+1] +1 - masks1[k:k+1] )
                file_path = '{:s}/results/masked_img_gray{:d}.png'.format(result_save_path, count)
                save_image(grid, file_path)
    
    def forward(self, masked_image, mask, masked_images_gray, gt_image):
        self.real_A = masked_images_gray
        ...
        self.mask1=mask[:,0:1,:,:]
        mask1=mask[:,0:1,:,:]
        self.comp_B = self.fake_B * (1 - mask1) + self.real_B * mask1
    ...
    def get_g_loss(self):
        ...
        comp_B = torch.cat((comp_B,comp_B,comp_B),1)
        real_B_feats = self.lossNet(real_B)
        fake_B_feats = self.lossNet(fake_B)
        comp_B_feats = self.lossNet(comp_B)
        
        tv_loss = self.TV_loss(comp_B * (1 - self.mask1))
        ...
        valid_loss = self.l1_loss(real_B, fake_B, self.mask1)
        hole_loss = self.l1_loss(real_B, fake_B, (1 - self.mask1))
